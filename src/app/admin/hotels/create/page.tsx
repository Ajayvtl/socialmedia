...
export default function CreateHotelPage() {
    ...
    useEffect(() => {
        // fetchPackages called via country dependency below
        fetchCountries();
    }, []);
    ...
    useEffect(() => {
        if (formData.country_id) {
            ...
            // Phase 10.5.2: Re-fetch packages to get localized pricing
            fetchPackages(formData.country_id);
        }
        else {
            setStates([]);
            fetchPackages(); // Fetch global if no country
        }
    }, [formData.country_id]);
    ...
    useEffect(() => {
        // Recalculate summary when package or billing cycle changes
        // dependent on packages being loaded
        calculateSummary();
    ...
    const fetchPackages = async (countryId: string | null = null) => {
        try {
            const endpoint = countryId ? `/saas/packages?country_id=${countryId}` : '/saas/packages';
            const res = await api.get(endpoint);
            setPackages(res.data.data);

            // Should we reset selection? Ideally keep if ID exists, else select first
            if (res.data.data.length > 0) {
                // Convert IDs to string for comparison
                const currentExists = res.data.data.find((p: any) => p.id == formData.package_id);
                if (!currentExists) {
    ...
    // Helper to get formatted price based on API response (which is now localized)
    const getPackageDetails = (pkg: any) => {
        // Backend now returns localized price/currency in the top-level fields
        const currencyMap: any = { 'USD': '$', 'INR': '₹', 'EUR': '€', 'GBP': '£' };
        const symbol = currencyMap[pkg.currency] || pkg.currency || '$';

        return {
            monthly: Number(pkg.price_monthly || 0),
            yearly: Number(pkg.price_yearly || 0),
            symbol: symbol,
    ...
    const calculateSummary = () => {
        ...
        if (isInclusive) {
            total = price;
            basic = total / (1 + (taxPercent / 100));
            tax = total - basic;
        } else {
            basic = price;
            tax = (basic * taxPercent) / 100;
            total = basic + tax;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success('Hotel & Subscription Created Successfully');
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 gap-6">
                            <input type="text" name="name" required value={formData.name} onChange={handleChange}
                                className={inputClass} placeholder="Hotel Name" />
