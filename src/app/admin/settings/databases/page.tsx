...
import { PlusIcon, TrashIcon, CircleStackIcon, PencilIcon, CheckCircleIcon, XCircleIcon, SignalIcon, PowerIcon, Squares2X2Icon, ListBulletIcon, ArrowDownTrayIcon, MagnifyingGlassIcon, ArrowUpTrayIcon, ArrowsRightLeftIcon } from "@heroicons/react/24/outline";
...
export default function DatabasesPage() {
    ...
    const canView = hasAnyPermission(user, ['database.view', 'database.manage', 'settings.view.global', 'menu.database_admin']);
    const canManage = hasAnyPermission(user, ['database.manage']);
    const canMoveToTemplate = hasAnyPermission(user, ['database.manage', 'template_db.manage']);
    const canTest = hasAnyPermission(user, ['database.test', 'database.manage']);
    const canMigrate = hasAnyPermission(user, ['database.migrate', 'database.manage']);
    const canExport = hasAnyPermission(user, ['database.export', 'database.manage']);
    const canImport = hasAnyPermission(user, ['database.import', 'database.manage']);

    const fetchData = async () => {
        try {
            const [dbRes, catRes] = await Promise.all([
                api.get('/saas/category-databases'),
    ...
    // Secure Action State
    const [actionToConfirm, setActionToConfirm] = useState<{ type: 'delete' | 'toggle' | 'edit' | 'export' | 'import' | 'migrate' | 'move_to_template', item: any } | null>(null);
    ...
    const initiateSecureAction = (type: 'delete' | 'toggle' | 'edit' | 'export' | 'import' | 'migrate' | 'move_to_template', item: any) => {
    ...
    const handleSecureAction = async () => {
        ...
        try {
            if (actionToConfirm.type === 'delete') {
                await api.delete(`/saas/category-databases/${actionToConfirm.item.id}`, {
                    data: { admin_password: securePassword, action_reason: actionReason || null }
                });
                toast.success("Deleted successfully");
            } else if (actionToConfirm.type === 'toggle') {
                await api.put(`/saas/category-databases/${actionToConfirm.item.id}`, {
                ...
                const response = await api.post(`/saas/category-databases/${actionToConfirm.item.id}/export`,
                ...
                await api.post(`/saas/category-databases/${actionToConfirm.item.id}/import`, formData, {
                ...
                const toastId = toast.loading("Running Mid DB migration...");
                await api.post(`/saas/category-databases/${actionToConfirm.item.id}/migrate`, {
                    admin_password: securePassword,
                    action_reason: actionReason || null
                });
                toast.dismiss(toastId);
                toast.success("Mid DB migration completed");
            } else if (actionToConfirm.type === 'move_to_template') {
                const toastId = toast.loading("Moving mapping to Template DB...");
                await api.post(`/saas/category-databases/${actionToConfirm.item.id}/move-to-template`, {
                    admin_password: securePassword,
                    action_reason: actionReason || null
                });
                toast.dismiss(toastId);
                toast.success("Moved to Template DB");
            }
    ...
    const runConnectionTestWithOptions = async (runMigration: boolean) => {
        ...
        try {
            ...
            const res = await api.post('/saas/category-databases/test', {
                ...formData,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.post('/saas/category-databases', {
                ...formData,
    ...
    const confirmEditSubmit = async () => {
        ...
        try {
            await api.put(`/saas/category-databases/${currentId}`, { ...formData, admin_password: securePassword, action_reason: actionReason || null });
        ...
        <div className="p-8 max-w-6xl mx-auto">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Manage runtime Mid DB mappings per Category and Subcategory</p>
                    {dbSourceIndicator && (
                            ...
                            {paginatedDatabases.map((item: any) => (
                                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                                    ...
                                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                                        ...
                                        {canMigrate && <button onClick={() => initiateSecureAction("migrate", item)} className="p-2 rounded bg-gray-50 dark:bg-slate-700 text-emerald-500 hover:text-emerald-700" title="Run Mid DB Migration"><CircleStackIcon className="w-4 h-4" /></button>}
                                        {canMoveToTemplate && <button onClick={() => initiateSecureAction("move_to_template", item)} className="p-2 rounded bg-gray-50 dark:bg-slate-700 text-violet-500 hover:text-violet-700" title="Move to Template DB"><ArrowsRightLeftIcon className="w-4 h-4" /></button>}
                        ...
                        <div key={item.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 flex flex-col hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="flex gap-1">
                                    ...
                                    {canMigrate && <button onClick={() => initiateSecureAction("migrate", item)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-700 rounded text-emerald-500 hover:text-emerald-700 transition-colors" title="Run Mid DB Migration"><CircleStackIcon className="w-4 h-4" /></button>}
                                    {canMoveToTemplate && <button onClick={() => initiateSecureAction("move_to_template", item)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-700 rounded text-violet-500 hover:text-violet-700 transition-colors" title="Move to Template DB"><ArrowsRightLeftIcon className="w-4 h-4" /></button>}
